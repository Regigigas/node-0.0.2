function UpdateEntry(container) {
  this.container = container;
  this.id = '';
  this.init();
}

UpdateEntry.ModelTemp = `
	<div class="modal fade js-updatepos-modal" role="dialog" aria-labelledby="UpdateEntryLabel">
	  <div class="modal-dialog" role="document">
	    <div class="modal-content">
	      <div class="modal-header">
	        <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
	        <h4 class="modal-title" id="UpdateEntryLabel">修改职位</h4>
	      </div>
...
$.extend(UpdateEntry.prototype, {
  init: function () {
    this.createDom();
    this.bindEvents();
  },
  createDom: function () {
    this.element = $(UpdateEntry.ModelTemp);
    this.companyElem = this.element.find(".js-company");
    this.entryElem = this.element.find(".js-position");
  ...
  showItem: function (id) {
    ...
    this.getEntryInfo(id);
  },
  getEntryInfo: function (id) {
    $.ajax({
      url: "/api/getEntry",
      data: {
        id: id
      },
      success: $.proxy(this.handleGetEntryInfoSucc, this)
    })
  },
  handleGetEntryInfoSucc: function (res) {
  ...
  handleSubmitBtnClick: function () {
    ...
    $.ajax({
      ...
      contentType: false,
      data: formData,
      success: $.proxy(this.handleUpdateEntrySucc, this)
    })
    
  },
  handleUpdateEntrySucc: function (res) {
    console.log(res);
